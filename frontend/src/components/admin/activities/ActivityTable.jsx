      ...
      {activities.length === 0 ? (
        ...
        <div className="divide-y divide-gray-100 max-h-[600px] overflow-y-auto">
          {activities.map((activity) => {
            ...
            return (
              <div key={activity._id || activity.id} className="p-6 hover:bg-gray-50 transition-colors duration-200 group">
                <div className="flex items-start space-x-4">
                  {/* Activity Icon */}
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-lg flex-shrink-0 ${getColorClass(config.color)} group-hover:scale-105 transition-transform duration-200 shadow-sm`}>
                    {config.icon}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center space-x-3">
                        ...
                        <span className={getSeverityBadge(config.severity)}>
                    ...
                    <p className="text-gray-700 mb-3 leading-relaxed text-sm">
                      {activity.description || transformed.description}
                    </p>

                    {/* Activity Metadata */}
                    <div className="flex flex-wrap items-center gap-3 text-sm text-gray-600">
                      <div className="flex items-center space-x-2 bg-gray-100 px-2.5 py-1 rounded-lg">
                    ...
                    {activity.metadata && Object.keys(activity.metadata).length > 0 && (
                      <div className="mt-3 p-3 bg-gray-50 rounded-lg border border-gray-200 text-xs">
                        <details className="cursor-pointer">
                          <summary className="text-gray-600 hover:text-gray-800 font-medium flex items-center space-x-2">
                          ...
                          <div className="mt-3 space-y-2">
                            {Object.entries(activity.metadata).map(([key, value]) => (
                              <div key={key} className="flex justify-between items-start">
                                <span className="font-medium text-gray-600 capitalize flex-shrink-0 mr-4">
